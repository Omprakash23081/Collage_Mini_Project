          ...
          <div className={`${style.navLinks} ${isMenuOpen ? style.open : ""}`}>
            ...
            <div className={style.Resource_navigation}>
              <div className={style.solution}>
                <Link to="/roadmaps" onClick={() => setIsMenuOpen(false)}>Roadmaps</Link>
              </div>
            </div>
            <div className={style.Company_navigation}>
              <div className={style.solution}>
                <a href="#yearBook" onClick={() => setIsMenuOpen(false)}>Features</a>
              </div>
            </div>
            {user && (
              <>
                <div className={style.solution}>
                  <Link to="/primum/canteen" onClick={() => setIsMenuOpen(false)}>Order</Link>
                </div>
                <div className={style.solution}>
                  <Link to="/primum/print" onClick={() => setIsMenuOpen(false)}>Print</Link>
                </div>
              </>
            )}
            ...
            <div className={style.mobileAuthSearch}>
              {!user ? (
                <NavLink to="/login" className={style.mobileLoginBtn} onClick={() => setIsMenuOpen(false)}>
                  Login
                </NavLink>
              ) : user.role === 'admin' ? (
                <div className={style.dualButtonsMobile}>
                  ...
                  <Link 
                    ...
                    onClick={(e) => { setIsMenuOpen(false); handlePrimumClick(e); }}
                  >
                    Primum
                  </Link>
                </div>
                ...
                <Link 
                  to={user.role === 'student' || user.role === 'teacher' ? "/primum" : (user.role === 'canteen_vendor' ? "/canteen" : "/stationery")}
                  className={style.mobileLoginBtn}
                  onClick={(e) => { setIsMenuOpen(false); handlePrimumClick(e); }}
                >
              ...
              <div className={style.mobileSearch}>
                 <div className={style.mobileInputWrapper}>
                   <div className={style.mobileSearchInputContainer}>
                     <FontAwesomeIcon icon={faSearch} className={style.searchIconSmall} />
                     <input 
                       type="search" 
                   ...
                   {isSearching && <span className={style.searchingText}>...</span>}
                 </div>
            ...
            <div className={style.notificationWrapper}>
              ...
              >
                ...
                {notifications.length > 0 && (
                  <span className={style.notificationBadge}>{notifications.length}</span>
                )}
              </div>
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((notif) => (
                      <div key={notif._id} className={style.notificationItem}>
                        ...
                        <p className={style.notifContent}>{notif.response.substring(0, 50)}...</p>
                        <button 
              ...
              <div className={style.authWrapper}>
                {user.role === 'admin' ? (
                  <div className={style.dualButtonsDesktop}>
                    <Link 
                      to="/home" 
                      className={style.externalAdminBtn}
                    >
                      <b>Dashboard</b>
                    </Link>
                    <Link to="/primum" className={style.internalPrimumBtn} onClick={handlePrimumClick}>
                      <b>Primum</b>
                    </Link>
                  </div>
                ) : (
                  <button className={style.navigation_Butten1}>
                    <Link to={
                      user.role === 'student' || user.role === 'admin' || user.role === 'teacher' ? "/primum" : 
                      user.role === 'canteen_vendor' ? "/canteen" :
                      user.role === 'stationery_vendor' ? "/stationery" :
                      "/home"
            ...
            <div className={style.searchContainerWrapper}>
              <div className={style.searchContainer}>
                <div className={style.navigation_Butten2}>
                  <FontAwesomeIcon icon={faSearch} className={style.searchIconDesktop} />
                  <input 
                    type="search" 
              ...
              {searchResults.length > 0 && (
                <div className={style.searchResultsDropdown}>
                  {searchResults.slice(0, 6).map((item, index) => (
                    <Link 
                      key={index} 
                      to={item.type === 'pyq' ? `/primum/pyq/${item.subject}` : `/primum/notes/${item.subject}`}
                    ...
                    >
                      ...
                      <div className={style.resultInfo}>
                        <span className={item.isPremium ? style.premiumTitle : style.resultTitle}>
                          {item.title || item.subject}
                        </span>
                        <span className={style.resultType}>{item.type.toUpperCase()} • {item.subject}</span>
                      </div>
                    </Link>
                  ))}
                </div>
